      ...
      <div className="group relative w-full rounded-[2rem] overflow-hidden border border-[#533afd]/10 shadow-sm hover:shadow-xl hover:shadow-[#533afd]/10 hover:border-[#533afd]/25 transition-all duration-500 grid grid-cols-1 md:grid-cols-[4fr_6fr] min-h-[340px]">
        ...
        <div className="relative overflow-hidden min-h-[300px] md:min-h-0 flex flex-col justify-between p-9 sm:p-11">
          ...
          <div className="relative z-10 flex items-start justify-between">
            ...
            <div className="inline-flex items-center px-3 py-1 bg-white/60 backdrop-blur-sm border border-[#533afd]/20 text-[#533afd] text-[10px] font-bold uppercase tracking-wider rounded-full shrink-0 shadow-sm">
              <Star size={12} className="mr-1.5 fill-[#533afd]/20" />
          ...
          <div className="relative z-10 mt-auto">
            {study.metrics && study.metrics.length > 0 && (
              <div className="mb-4">
                <div className="text-4xl md:text-[2.75rem] font-bold text-[#533afd] tracking-tight leading-none mb-2" style={{ fontFamily: 'var(--font-inter)' }}>
                  {study.metrics[0].metric_value}
                </div>
                <div className="text-[13px] font-semibold text-[#0d253d] max-w-[200px] leading-snug" style={{ fontFamily: 'var(--font-inter)' }}>
                  {study.metrics[0].metric_label}
                </div>
              </div>
            )}
            
            <p className={cn("text-[15px] text-[#0d253d] leading-relaxed max-w-xs", study.metrics && study.metrics.length > 1 ? "mb-4" : "")} style={{ fontFamily: 'var(--font-quicksand)' }}>
            ...
            {study.metrics && study.metrics.length > 1 && (
              <div className="flex flex-wrap items-center gap-2">
                {study.metrics.slice(1).map((m: any, i: number) => (
                  <div key={i} className="inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-white/80 backdrop-blur-sm border border-[#533afd]/10 rounded-full text-[12px] shadow-sm">
        ...
        <div className="relative bg-white flex flex-col justify-between p-9 sm:p-11 border-t md:border-t-0 md:border-l border-slate-100">
          ...
          {/* Title */}
          <h2 className="text-lg md:text-2xl font-bold leading-tight tracking-wide text-[#0d253d] mb-4 group-hover:text-[#533afd] transition-colors" style={{ fontFamily: 'var(--font-inter)' }}>
            {study.title}
          </h2>

          {/* Description / subtitle */}
          {(study.description || study.subtitle) && (
            <p className="text-base text-[#0d253d] leading-relaxed mb-5 line-clamp-3" style={{ fontFamily: 'var(--font-quicksand)' }}>
              {study.description || study.subtitle}
            </p>
          )}
          ...
          <div className="mt-auto pt-6 border-t border-slate-100">
            ...
            {study.metrics && study.metrics.length > 0 && (
              <div className={cn("grid grid-cols-3 gap-4", study.testimonial?.quote ? "mb-6" : "")}>
                {study.metrics.slice(0, 3).map((m: any, i: number) => (
                  <div key={i}>
                    <div className="text-2xl sm:text-3xl font-bold text-[#0d253d] leading-none mb-1.5" style={{ fontFamily: 'var(--font-inter)' }}>
                      {m.metric_value}
                    </div>
            ...
            {study.testimonial?.quote && (
              <div className={cn("flex items-start gap-3", study.metrics && study.metrics.length > 0 ? "pt-5 border-t border-slate-100/60" : "")}>
                ...
                <div>
                  <p className="text-[13px] text-[#0d253d] italic leading-snug mb-2 line-clamp-2" style={{ fontFamily: 'var(--font-quicksand)' }}>
                    "{study.testimonial.quote}"
                  </p>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[11px] font-semibold text-[#0d253d]">{study.testimonial.person_name}</span>
            ...
            <div className="pt-5 mt-5 border-t border-slate-100 flex items-center justify-between text-[#533afd] font-bold text-[14px]" style={{ fontFamily: 'var(--font-inter)' }}>
...
function StudyCard({ study, content }: { study: any; content: any }) {
  ...
  return (
    <Link href={`/case-studies/${study.slug}`} className="group block h-full">
      <div className="relative h-full flex flex-col sm:flex-row rounded-[1.5rem] border border-slate-200/80 overflow-hidden bg-white shadow-sm hover:shadow-xl hover:border-[#533afd]/20 transition-all duration-300">
        ...
        <div className="relative w-full sm:w-[40%] min-h-[240px] sm:h-auto shrink-0 overflow-hidden flex flex-col justify-between p-7 border-b sm:border-b-0 sm:border-r border-slate-100">
          ...
          <div className="relative z-10 mt-auto pt-8">
            {study.metrics && study.metrics.length > 0 && (
              <div className="mb-4">
                <div className="text-3xl font-bold text-[#533afd] tracking-tight leading-none mb-1.5" style={{ fontFamily: 'var(--font-inter)' }}>
                  {study.metrics[0].metric_value}
                </div>
                <div className="text-[12px] font-medium text-[#0d253d] max-w-[180px] leading-snug" style={{ fontFamily: 'var(--font-inter)' }}>
                  {study.metrics[0].metric_label}
                </div>
              </div>
            )}
            
            <p className={cn("text-[14px] text-[#0d253d] leading-relaxed max-w-[200px]", study.metrics && study.metrics.length > 1 ? "mb-4" : "")} style={{ fontFamily: 'var(--font-quicksand)' }}>
              {study.subtitle || study.title}
            </p>
            ...
            {study.metrics && study.metrics.length > 1 && (
              <div className="flex flex-wrap items-center gap-2">
                {study.metrics.slice(1).map((m: any, i: number) => (
                  <div key={i} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white/80 backdrop-blur-sm border border-[#533afd]/10 rounded-full text-[11px] shadow-sm">
                    <span className="font-bold text-[#533afd]">{m.metric_value}</span>
                    <span className="text-[#64748b]">{m.metric_label}</span>
                  </div>
                ))}
        ...
        <div className="flex flex-col flex-1 p-6 sm:p-8">
          ...
          <h3 className="text-[1.25rem] font-bold text-[#0d253d] leading-snug mb-3 group-hover:text-[#533afd] transition-colors tracking-tight" style={{ fontFamily: 'var(--font-inter)' }}>
            {study.title}
          </h3>
          ...
          <div className="mt-auto">
            ...
            {study.metrics && study.metrics.length > 0 && (
              <div className={cn("flex items-start gap-8", study.testimonial?.quote ? "mb-5" : "mb-6")}>
                {study.metrics.slice(0, 3).map((m: any, i: number) => (
                  <div key={i}>
                    <div className="text-2xl font-bold text-[#0d253d] mb-1" style={{ fontFamily: 'var(--font-inter)' }}>{m.metric_value}</div>
            ...
            {study.testimonial?.quote && (
              <div className="flex items-start gap-2.5 mb-5 pt-4 border-t border-slate-50">
                ...
                <div>
                  <p className="text-[12px] text-[#0d253d] italic leading-snug mb-1.5 line-clamp-2" style={{ fontFamily: 'var(--font-quicksand)' }}>
                    "{study.testimonial.quote}"
                  </p>
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[10px] font-semibold text-[#0d253d] truncate max-w-[100px]">{study.testimonial.person_name}</span>
                  </div>
                </div>
              </div>
            )}

            <div className="pt-5 border-t border-slate-100 flex items-center justify-between text-[#533afd] font-bold text-[14px]" style={{ fontFamily: 'var(--font-inter)' }}>
              <span>{content?.labels?.read_more || 'See Full Story'}</span>
              <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
            </div>
          </div>
