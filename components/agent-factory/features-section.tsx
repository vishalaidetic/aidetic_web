...
function DataSourcesMockup() {
  const sources = [
    ...
    { name: 'Salesforce', status: true, color: 'bg-[#533afd]' },
  ...
  return (
    <div className="flex flex-col w-full h-full bg-white">
      {/* Header */}
      <div className="px-5 py-4 border-[#533afd] border-slate-100">
        <div className="flex items-center justify-between">
          <span className="text-xs font-bold text-[#0d253d]" style={{ fontFamily: 'var(--font-inter)' }}>Data Connectors</span>
          <span className="text-[9px] px-2 py-0.5 bg-green-50 text-green-600 border border-green-200 rounded-full">5 Active</span>
        </div>
        <p className="text-[9px] text-[#64748d] mt-1">Unified access across all enterprise sources</p>
      </div>
        ...
        {sources.map((s, i) => (
          <div key={i} className="flex items-center gap-3 p-3 rounded-xl border border-slate-100 bg-slate-50/50 hover:bg-white hover:shadow-sm transition-all">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-[10px] font-semibold text-[#0d253d] truncate">{s.name}</div>
              <div className={`text-[8px] ${s.status ? 'text-green-500' : 'text-[#64748d]'}`}>
      ...
      <div className="px-5 py-3 border-t border-slate-100 flex items-center gap-2">
        <div className="flex-1 h-1.5 rounded-full bg-slate-100 overflow-hidden">
          <div className="h-full w-[83%] bg-gradient-to-r from-[#533afd] to-blue-500 rounded-full" />
        </div>
        <span className="text-[9px] text-[#64748d]">83% coverage</span>
      </div>
    </div>
  )
}
...
function SecurityMockup() {
  ...
  return (
    <div className="flex flex-col w-full h-full bg-white">
      {/* Certs row */}
      <div className="px-5 pt-5 pb-4 border-[#533afd] border-slate-100">
        <div className="text-[9px] font-bold text-[#64748d] uppercase tracking-widest mb-3" style={{ fontFamily: 'var(--font-inter)' }}>Compliance Certifications</div>
        <div className="flex gap-3">
          {certs.map((c, i) => (
            <div key={i} className="flex flex-col items-center gap-1 px-3 py-2 rounded-xl border-2 border-slate-200 bg-slate-50 hover:border-[#533afd] transition-colors">
              <Shield className="w-4 h-4 text-[#64748d]" strokeWidth={1.5} />
              <span className="text-[7px] font-bold text-[#64748d] text-center leading-tight">{c}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex-1 p-5 space-y-2">
        ...
        <div className="flex items-center gap-3 px-3 py-1.5 bg-slate-50 rounded-lg">
          ...
          <span className="flex-1 text-[9px] font-semibold text-[#64748d]">Role Name</span>
          <span className="w-28 text-[9px] font-semibold text-[#64748d]">Description</span>
        </div>

        {/* Rows */}
        {roles.map((r, i) => (
          <div key={i} className={`flex items-center gap-3 px-3 py-2 rounded-lg transition-colors ${r.active ? 'bg-[#533afd]' : 'hover:bg-slate-50'}`}>
            ...
            <span className={`flex-1 text-[10px] font-medium ${r.active ? 'text-white' : 'text-[#0d253d]'}`}>{r.name}</span>
            <span className={`w-28 text-[9px] ${r.active ? 'text-white/80' : 'text-[#64748d]'}`}>{r.access}</span>
          </div>
        ))}
      </div>
      ...
      {/* Lock footer */}
      <div className="px-5 py-3 border-t border-slate-100 flex items-center gap-2">
        <Lock className="w-3 h-3 text-[#64748d]" strokeWidth={1.5} />
        <span className="text-[9px] text-[#64748d]">Zero LLM data retention · Row-level security active</span>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="flex flex-col w-full h-full bg-white">
      ...
      <div className="flex-1 flex items-center justify-center p-6 relative overflow-hidden">
        ...
        {/* Centre */}
        <div className="absolute w-20 h-20 rounded-3xl bg-[#533afd] shadow-xl shadow-cyan-200 flex items-center justify-center z-20" style={{ left: '50%', top: '50%', transform: 'translate(-50%, -50%)' }}>
        ...
        {tools.map((t, i) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-[10px] font-bold text-[#64748d] whitespace-nowrap">{t.name}</span>
            </div>
          )
        })}
      </div>
